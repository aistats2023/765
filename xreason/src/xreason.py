...
if __name__ == '__main__':
    ...
    if options.from_py_learning_explainer is not None:
      options.files = [options.from_py_learning_explainer]
    ...
    if options.files:
        ...
        if options.instance_file is not None:
          f = open(options.instance_file)
          data = json.loads(json.load(f))
          options.explain = data['data_instance']
          f.close()
          options.explain = [float(feature) for feature in options.explain]
        else:
