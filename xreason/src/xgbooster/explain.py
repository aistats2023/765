...
class MXExplainer(object):
    ...
    def prepare(self, sample):
        ...
        if self.verbose:
            ...
            #print('  explaining:  "IF {0} THEN {1}"'.format(' AND '.join(self.preamble), self.output))
            print("initial size:", len(self.preamble))
    def explain(self, sample, smallest, expl_ext=None, prefer_ext=False):
        """
            Hypotheses minimization.
        """
    ...
    def core_to_features(self, core):
      hyps = list(reduce(lambda x, y: x + self.hypos[y[0]:y[1]+1], [self.fcats[c] for c in core], []))
      expl = sorted(set(map(lambda v: self.v2feat[v], hyps)))
      return expl
    ...
    def extract_mus(self, reduce_='lin', start_from=None):
            ...
            def _assump_needed(a):
                if len(to_test) > 1:
                    ...
                    if not self.oracle.get_coex(self._cats2hypos(to_test), early_stop=True):
                        self.current_solution = list(to_test)
                        print("new best solution:", self.core_to_features(self.current_solution))
                        return False
                    to_test.add(a)
                    return True
                else:
        ...
        save_handler = signal.getsignal(signal.SIGINT)

        def new_handler(frame, signum):
          print("sigint solution:", self.core_to_features(self.current_solution))
          exit(0)

        signal.signal(signal.SIGINT, new_handler)
        start_from = None
        if self.optns.start_from_file is not None:
          assert os.path.exists(self.optns.start_from_file), "The file does not exist: " + self.optns.start_from_file
          f = open(self.optns.start_from_file)
          data = json.loads(json.load(f))
          start_from_features = data['features']
          f.close()
          lits = self.xgb.mxe.features_to_literals(start_from_features, self.sample)
          start_from = [i for i, h in enumerate(self.hypos) if h in lits]
          hypos_reduced = [h for h in self.hypos if h in lits]
        ...
        if start_from is None:
            ...
            print("over solution intern:", self.core_to_features(core))
            print("core:", core)
        else:
            #assert self.oracle.get_coex(hypos_reduced, full_instance=True, early_stop=True) == None, 'No prediction'
            #core = self.oracle.get_reason(self.v2cat)
            
            core = start_from
            print("over solution extern:", self.core_to_features(core))

        self.current_solution = core
        ...
        print("final solution:", self.core_to_features(self.current_solution))  
        signal.signal(signal.SIGINT, save_handler)  # Reset the right SIGINT
        return expl
