...
class XGBooster(object):
    ...
    def __init__(self, options, from_data=None, from_model=None,
        ...
        if from_data:
            ...
            fname = from_model
            if options.from_py_learning_explainer is not None:
              self.load_datainfo_py_learning_explainer(from_model)
            else:
    ...
    def load_datainfo_py_learning_explainer(self, model_directory):
        #get the files
        if os.path.isfile(model_directory):
          files = [(model_directory, model_directory.replace(".model", ".map"))]
        ...
        for _, model in enumerate(files):
          ...
          #load some data information

          #recuperate map
          f = open(map_file)
          data = json.loads(json.load(f))
          self.nb_features = data['n_features']
          self.num_class = data["n_labels"]
          self.use_categorical = False
          self.feature_names = [str(i) for i in range(self.nb_features)]
          self.target_name = list(range(self.num_class))
          self.categorical_features = []
          f.close()
